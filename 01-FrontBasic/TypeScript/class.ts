...
//클래스는 클래스를 상속받아 기능과/속성을 확장(extends) 할 수 있다.
...
//속성과 기능을 클래스내에서 구현해야합니다.
class Car implements Movable {
    speed: number;

    //생성자 함수:생성자함수는 클래스를 통해 객체가 생성된느 시점에 자동으로 호출되는 함수
    //클래스를 이용해 객체를 만들어 내는 과정 new Car()을 인스턴스화 한다라고 표현한다.
    constructor(speed: number) {
        //this는 현재 클래스내부에 접근하기 위한 예약어이고
        //this.speed는 클래스내 내부속성인 상단에 클래스내에 정의된 speed속성이고
        //뒤에 오는 speed는 객체를 생성하는 시점에 new Car(500); 생성자함수로 전달되는 속도값 파라메터
        this.speed = speed;
    }

    move() {
        console.log(`현재 자동차 ${this.speed.toString()} km 속도로 이동중입니다.`);
    };
...
//프로그램이 실행되는 컴퓨터의 메모리에 저장되며 이러한 과정을 인스턴스화 라고 한다.
//클래스는 인스턴스화 될 때 클래스 내 정의된 생성자함수가 자동으로 실행되고 해당 함수에 파라메터를 전달할 수 있다.
...
class User {
    ...
    private config() {
        ...
        console.log("개인화 정보가 설정되었습니다. 메일로 관련내용을 전송했습니다.")
    }
...
let user = new User("이형진", "1234", "test@test.co.kr");
user.greeting();
...
//접근제어자가 private 선언되어 있어서 객체가 생성된 후 외부에서 접근불가함:에러발생
//user.password="1234";
...
class AdminUser extends User {
    ...
    constructor(name: string, password: string, email: string, adminRole?: AdminRole) {
        ...
        //super메소드는 부모클래스의 생성자함수를 호출하는 것..
