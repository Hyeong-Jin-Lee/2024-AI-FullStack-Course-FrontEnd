...
//*기본적으로 next.js의 모든 컴포넌트 파일은 JSX만 표시할 때는 SSG방식으로 작동합니다. */
...
//restapi를 호출하여 빌드 타임의 데이터를 기준으로 정적 웹페이지를 생성해주는 함수
//SSG실행순서 : 개발환경 npm run build 를 통한 빌드실행->모든 컴포넌트파일중에서 getStaticProps()사용 컴포넌트 탐색
...
//   //백엔드에서 게시글 데이터를 조회해와서 해당 컴포넌트의 props데이터로 파라메터 형식으로 전달한다.
...
//서버에서 다시 생성된 정적 웹페이지를 다운받아 화면을 갱신한다.
export const getStaticProps = async () => {
  //백엔드에서 게시글 데이터를 조회해와서 해당 컴포넌트의 props데이터로 파라메터 형식으로 전달한다.
  const res = await fetch("http://localhost:5000/api/article/list");
  const result = await res.json();
  ...
  return {
    ...
    revalidate: 30, //페이지의 유효기간(수명)을 초단위로 지정
  };
};
