...
const ReducerCount = () => {
  //useReducer훅 생성하기
  //useReduce(리듀서함수-재사용/통합 로직처리함수, 초기데이터값);
  //useReducer()함수는 관리하는 상태값과 해당 리듀서함수를 호출하는 디스패치함수를 반환합니다.
  //dispatch의 의미는 이벤트 발생시 해당 이벤트를 처리해주는 함수를 의미
  //dispatch함수명은 임의로 지정한다. ex) data, dispatchData
  //UI이벤트 발생->디스패치함수 호출->리듀서함수실행->상태값이 변경됨->화면이 변경된 값으로 렌더링됨
    ...
    <div>
      ...
      <div className="text-center mt-6">
        <button
          ...
          className="ml-3 className=ml-3 rounded-md bg-white px-2.5 py-1.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50"
        >
          증가
        </button>
        <button
